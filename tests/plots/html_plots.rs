...
fn custom_colors_html() -> fmt::Result {
    ...
    let l1 = plot("cos").line2(x.iter().copied().zip_output(|x| x.cos()));
    let l2 = plot("sin-10").histogram2(x.iter().copied().step_by(3).map(|x| [x, x.sin() - 10.]));
...
"###;
    ...
    let s = poloto::plots!(
        ...
        plot("sin-3").histogram2(x.iter().copied().step_by(3).zip_output(|x| x.sin() - 3.)),
        plot("sin").scatter2(x.iter().copied().step_by(3).zip_output(|x| x.sin()))
    );
