...
fn dark() -> fmt::Result {
    ...
    let data = poloto::plots!(
        plot(format_move!("test {}", 1)).line(x.iter().copied().zip_output(f64::cos)),
        plot(format_move!("test {}", 2)).line(x.iter().copied().zip_output(f64::sin))
    );
...
fn no_plots() -> fmt::Result {
    let v: Vec<PlotRes<std::iter::Empty<PlotTag<(i128, i128),&'static str>>, (i128, i128)>> = vec![];
