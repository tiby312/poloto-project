...
fn main(){
    ...
    let theme=theme.append(".poloto1.poloto_histo.poloto_imgs{fill:red;stroke:black;stroke-width:2px}");

    // Say you want to style the text of the first legend
    let theme=theme.append(".poloto1.poloto_legend.poloto_text{fill:blue;}");

    // Say you want to style all line plots but not legend img.
    let theme=theme.append(".poloto_line.poloto_plot{stroke-width:20px}");
    ...
    let data = poloto::plots!(
        ...
        plot("sin-5").scatter().buffered(x.clone().step_by(3).zip_output(|x|x.sin()-5.))
    );

    poloto::data(data).build_and_label((
        "Demo: you can change the style of the svg file itself!",
        "x",
        "y",
    )).append_to(poloto::header().append(theme)).render_stdout();
