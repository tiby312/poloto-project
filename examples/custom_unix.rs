...
fn main() {
    ...
    let data = [
        (
            DayTimestamp(UnixTime::from_ymd_hms(day1, 23, 30, 59)),
            3144000,
        ),
        (DayTimestamp(UnixTime::from_ymd_hms(day2, 1, 2, 0)), 3518000),
        (DayTimestamp(UnixTime::from_ymd_hms(day2, 1, 5, 1)), 3835000),
        (
            DayTimestamp(UnixTime::from_ymd_hms(day2, 1, 30, 59)),
            2133000,
        ),
        (
            DayTimestamp(UnixTime::from_ymd_hms(day2, 1, 50, 1)),
            4133000,
        ),
    ];

    let x = poloto::ctx::<UnixTime>()
        .with_tick(|w, d| write!(w, "{}", d.format("%D")))
        .with_where(|w, d| write!(w, "{}", d.format("%m")))
        .with_marker(0);
