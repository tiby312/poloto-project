use tagu::{elem::Locked, prelude::Elem};
...
//     let mut s = String::new();
//     tagu::render_escapable(elem.inline(), &mut s).unwrap();
...
pub fn evcxr_display_img<R: Elem>(elem: R) {
    ...
    let r = tagu::build::single("img").with(("src", s));
    let mut s = String::new();
    tagu::render(r, &mut s).unwrap();
    evcxr_disp(s);
}
